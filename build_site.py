...
function render(id,push){
  ...
  contentEl.querySelectorAll('a').forEach(function(a){
    ...
    if(m.endsWith('.md')){
      var tid=m.slice(0,-3);
      if(ids[tid]){a.addEventListener('click',function(e){e.preventDefault();go(tid);});}
    }
  });
...
render(start,false);
window.addEventListener('hashchange',function(){var h=location.hash.replace('#','');if(ids[h])render(h,false)});
</script>
</body>
</html>"""
